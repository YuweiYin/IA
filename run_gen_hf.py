...
class LMGen:
    ...
    def run_inference(
        ...
        for batch_idx, cur_batch_data_items in enumerate(batches_data_items):
            ...
            batch_prompts = []
            for eval_item in batch_eval_items:
                cur_dialog = eval_item["dialog"]
            ...
            # At last, based on the reasoning, give the final answer (no more analysis)
            input_prompts_final = [_prompt + _analysis + "\n\nTherefore, the final answer is: " for
                                   _prompt, _analysis in zip(batch_prompts, batch_analysis)]
            gen_results_final = ModelUtils.open_model_gen(
                inputs=input_prompts_final, model=self.model, tokenizer=self.tokenizer, need_tokenize=True,
                max_new_tokens=32,
                temperature=0.0, top_p=None, top_k=None,
                # temperature=self.gen_temperature, top_p=self.top_p, top_k=self.top_k,
            )
            assert len(gen_results_final) == len(batch_prompts), len(gen_results_final)
            batch_pred_answer = [str(gen_result["output_text"]).strip() for gen_result in gen_results_final]

            break_flag = False
            for eval_item, prompt, analysis, pred_answer in zip(
                    batch_eval_items, batch_prompts, batch_analysis, batch_pred_answer):
                item_id_key = f"{item_idx}"
                # if item_id_key in done_ids:
                #     # self.logger.info(f"[Skip] this item has been processed already: {item_id_key}")
                #     continue
                analysis = re.sub(r"[^\x00-\x7F]+", "", analysis).strip()  # remove non-ASCII
                ...
                if item_idx >= max_num_eval > 0:
                    # self.save_jsonl(output_fp, all_results, mode="w", verbose=True)
                    break_flag = True
                    break
