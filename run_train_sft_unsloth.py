...
class SFTTraining:
    ...
    def __init__(
        ...
        training_data_fn = f"ft_data_{training_data_type}_training.jsonl"
        ...
        valid_data_fn = f"ft_data_{training_data_type}_validation.jsonl"
        valid_data_fp = os.path.join(
            training_data_dir, label_model_name, training_task_name, valid_data_fn)
    ...
    def finetune(
        ...
        if isinstance(self.model_ckpt_dir, str) and os.path.isdir(self.model_ckpt_dir):
            ...
            model = ModelUtils.initialize_model_hf_unsloth(
                model_name=self.model_name, cache_dir=self.cache_dir, verbose=self.verbose,
                do_train=True, do_4bit=False, model_ckpt_dir=self.model_ckpt_dir, max_seq_len=self.max_seq_len,
            )
...
def main(
    ...
    if isinstance(model_ckpt_dir, str) and os.path.isdir(model_ckpt_dir):
        ...
        if isinstance(ckpt_root_dir, str) and len(ckpt_root_dir) > 0:
            os.makedirs(ckpt_root_dir, exist_ok=True)
            ckpt_save_dir = os.path.join(ckpt_root_dir, "ckpt_ia_sft_unsloth", run_id, f"{cur_time}")
        else:
            ckpt_save_dir = os.path.join(project_root_dir, "ckpt_ia_sft_unsloth", run_id, f"{cur_time}")
