...
class Trainer:
    ...
    def _evaluate_function(self, model):
        ...
        valid_task_name = self.extra_kwargs["training_task_name"]
        # valid_training_data_dir = self.extra_kwargs["training_data_dir"]
        valid_training_data_type = self.extra_kwargs["training_data_type"]
        # valid_label_model_name = self.extra_kwargs["label_model_name"]
        valid_max_new_gen = self.extra_kwargs["max_new_gen"]
        valid_gen_temperature = self.extra_kwargs["gen_temperature"]
        # valid_ratio = self.extra_kwargs["valid_ratio"]
        ...
        # elif isinstance(valid_ratio, float) and 0.0 < valid_ratio < 1.0:
        #     valid_num = int(len_valid_all * valid_ratio)
        #     valid_data = random.sample(valid_data_all, valid_num)
        else:
        ...
        # valid_task_obj = None
        valid_eval_obj = None
        # valid_tokenizer = self.processing_class  # Trainer.tokenizer is now deprecated
        # valid_tokenizer.pad_token = valid_tokenizer.eos_token
        # valid_tokenizer.pad_token_id = valid_tokenizer.eos_token_id
        # # valid_terminators_gen = [
        # #     valid_tokenizer.eos_token_id,
        # #     # valid_tokenizer.convert_tokens_to_ids("<|eot_id|>")
        # #     valid_tokenizer.convert_tokens_to_ids(valid_tokenizer.eos_token)
        # # ]
        ...
        for item_dict in valid_data:
            ...
            # messages = item_dict["messages"]
            common_info_dict = item_dict["common_info_dict"]
            assert isinstance(common_info_dict, dict), type(common_info_dict)
            # cur_method = common_info_dict["method"]
            # cur_model = common_info_dict["model"]
            # cur_dataset_name = common_info_dict["dataset_name"]
            # cur_subset_name = common_info_dict["subset_name"]
            # cur_split_name = common_info_dict["split_name"]
            # cur_item_id_key = common_info_dict["item_id_key"]
            # cur_ds_id = common_info_dict["ds_id"]
            # cur_batch_idx = common_info_dict.get("batch_idx", 0)
            # cur_item_idx = common_info_dict["item_idx"]
            cur_user_dialog = common_info_dict["dialog"]  # user prompt: List[Dict[str, str]]
            cur_answers = common_info_dict["answers"]  # valid answers: List[str]
            assert isinstance(cur_answers, list) and len(cur_answers) > 0, type(cur_answers)
            cur_info = common_info_dict["info"]
            ...
            if "ia" in valid_training_data_type:
                # assert "ia" in PromptingMethods
                cur_prompt += "\n" + str(PromptingMethods["ia"]).strip()
        ...
        # if skip_cnt > 0:
        #     valid_logger.info(f">>> !!! >>> `_evaluate_function`: skip_cnt = {skip_cnt}")
        ...
        # for cur_prompt_dict in all_prompts:
        # for cur_prompt_dict in tqdm.tqdm(all_prompts, desc=f"Validation (bsz={valid_bsz})"):
        # for cur_prompt_batch in tqdm.tqdm(prompt_batches, desc=f"Validation (bsz={valid_bsz})"):
        for cur_batch_idx, cur_prompt_batch in enumerate(prompt_batches):
            ...
            for gen_res, cur_prompt_dict in zip(gen_results, cur_prompt_batch):
                ...
                gen_res["info"] = cur_prompt_dict["info"]

                assert "output_text" in gen_res, gen_res.keys()
                cur_output_text = str(gen_res["output_text"])
                cur_output_text = re.sub(r"[^\x00-\x7F]+", "", cur_output_text).strip()  # remove non-ASCII
                gen_res["analysis"] = cur_output_text
                gen_res["pred_answer"] = cur_output_text
        ...
        # avg_valid_score = sum(all_scores) / len(all_scores) if len(all_scores) > 0 else 0.0
    ...
    def _maybe_log_save_evaluate(
        ...
        if self.control.should_save or do_valid:
            # ### IA ### Evaluation on the validation set
